...
@router.get("/")
def get_courses(db:Session = Depends(get_db), user_id: int =  Depends(oauth2.get_current_user)):
...
def create_course(course : schemas.Course,db: Session = Depends(get_db), user_id: int =  Depends(oauth2.get_current_user)):
    # cursor.execute("""
    #                 INSERT INTO courses 
...
@router.get("/{id}")
def get_id_course(id:int, db:Session = Depends(get_db), user_id: int =  Depends(oauth2.get_current_user)):
    # cursor.execute("""
    #                 SELECT * FROM courses 
    #                 WHERE id = %s
    #             """,[str(id)])
...
def delete_course(id:int, db: Session = Depends(get_db), user_id: int =  Depends(oauth2.get_current_user)):
    # cursor.execute("""
    #                DELETE FROM courses
    #                WHERE id = %s
    #                RETURNING *
...
def update_course(id:int, course: schemas.Course, db :Session = Depends(get_db), user_id: int =  Depends(oauth2.get_current_user)):
#     cursor.execute("""
#                    UPDATE courses SET
#                    course_title = %s,
#                    course_id = %s,
...
def get_professors_of_course_id(course_id: int, db: Session = Depends(get_db)):
    ...
    profs = db.query(models.Prof_Course).filter(models.Prof_Course.course_id == course_id)
...
@router.post("/professor-teaching", response_model=schemas.ProfCourse)
